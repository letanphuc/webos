...
void ssh_service_start(void)
{
	...
	if (ret == -ENOENT) {
		LOG_INF("SSH host key not found at %s; push keys to %s first",
			ssh_host_key_path, ssh_key_dir);
		return;
	}
	if (ret != 0) {
	...
	ret = ssh_server_start(sshd, net_sad(&bind_addr), 0, CONFIG_WEBOS_SSH_USERNAME,
			       CONFIG_WEBOS_SSH_PASSWORD,
			       NULL, 0, shell_sshd_event_callback,
			       shell_sshd_transport_event_callback, NULL);
	if (ret != 0) {
