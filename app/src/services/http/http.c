...
void set_json_response(struct http_response_ctx* response_ctx, enum http_status status, const char* body,
                       size_t body_len) {
...
void set_text_response(struct http_response_ctx* response_ctx, enum http_status status, const char* body,
                       size_t body_len) {
  response_ctx->status = status;
...
HTTP_SERVICE_DEFINE(webos_http_service, NULL, &webos_http_port, CONFIG_HTTP_SERVER_MAX_CLIENTS, 4, NULL, NULL, NULL);
