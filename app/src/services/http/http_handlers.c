...
static int push_handler(struct http_client_ctx* client, enum http_transaction_status status,
  ...
  if (cursor + request_ctx->data_len >= sizeof(body)) {
    ...
    set_json_response(response_ctx, HTTP_413_PAYLOAD_TOO_LARGE, too_large, strlen(too_large));
    return 0;
  }
...
static int pushbin_handler(struct http_client_ctx* client, enum http_transaction_status status,
                           const struct http_request_ctx* request_ctx, struct http_response_ctx* response_ctx,
                           void* user_data) {
  ARG_UNUSED(client);
  ARG_UNUSED(user_data);
  ...
  if (!path[0] && request_ctx->headers && request_ctx->header_count > 0) {
    for (size_t i = 0; i < request_ctx->header_count; i++) {
      ...
      if (strcmp(header->name, "X-Webos-Path") == 0 && header->value != NULL) {
  ...
  if (cursor + request_ctx->data_len >= sizeof(body)) {
    LOG_ERR("pushbin: request too large");
    cursor = 0;
    path[0] = '\0';
    set_json_response(response_ctx, HTTP_413_PAYLOAD_TOO_LARGE, "{\"error\":\"too large\"}\n", 20);
    return 0;
  }
  ...
  if (!path[0]) {
    ...
    if (!newline) {
      cursor = 0;
      set_json_response(response_ctx, HTTP_400_BAD_REQUEST, "{\"error\":\"no path\"}\n", 19);
      return 0;
    }
    size_t path_len = newline - body;
    if (path_len >= sizeof(path)) {
      cursor = 0;
      set_json_response(response_ctx, HTTP_400_BAD_REQUEST, "{\"error\":\"bad path\"}\n", 19);
      return 0;
    }
  ...
  if (ret == 0) {
    ...
    set_json_response(response_ctx, HTTP_500_INTERNAL_SERVER_ERROR, "{\"error\":\"write failed\"}\n", 23);
  }
  path[0] = '\0';
  return 0;
}
...
static int ota_handler(struct http_client_ctx* client, enum http_transaction_status status,
  ...
  if (request_ctx->data_len > 0) {
    ret = ota_write(request_ctx->data, request_ctx->data_len, status == HTTP_SERVER_REQUEST_DATA_FINAL);
  }
  ...
  if (ret == 0 && status == HTTP_SERVER_REQUEST_DATA_FINAL) {
    ...
    if (ret == 0) {
      ret = snprintk(response, sizeof(response), "{\"ok\":true,\"bytes\":%zu,\"reboot_ms\":%d}\n", written,
...
static int log_handler(struct http_client_ctx* client, enum http_transaction_status status,
                       const struct http_request_ctx* request_ctx, struct http_response_ctx* response_ctx,
                       void* user_data) {
  ARG_UNUSED(client);
  ARG_UNUSED(user_data);
...
static struct http_resource_detail_dynamic push_resource_detail = {
    .common = {.bitmask_of_supported_http_methods = BIT(HTTP_POST), .type = HTTP_RESOURCE_TYPE_DYNAMIC},
    .cb = push_handler,
};
...
HTTP_RESOURCE_DEFINE(pushbin_resource, webos_http_service, "/pushbin", &pushbin_resource_detail);
