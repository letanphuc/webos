...
static void make_path(char* buf, int sz, int pin, const char* suffix) {
  const char* prefix = "/dev/gpio/";
  const char* p;
  int i = 0;
...
int main(int argc, char** argv) {
  ...
  webos_write(path, "out", 3);

  for (int i = 0; i < count; i++) {
    make_path(path, sizeof(path), pin, "/value");
    webos_write(path, "1", 1);
    log_print("ON");

    webos_sleep_ms(500);

    webos_write(path, "0", 1);
    log_print("OFF");
  ...
  make_path(path, sizeof(path), pin, "/value");
  int bytes_read = webos_read(path, buf, sizeof(buf) - 1);
  if (bytes_read >= 0) {
    buf[bytes_read] = '\0';
    log_print(buf);
  }
